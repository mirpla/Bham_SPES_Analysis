...
function vert_rain_plotbeh(dat,col,dotsize,smooth,Alpha);
...
y = linspace(0,0);
plot(x,y,'linewidth',1,'color',col,'linestyle', '--');
hold on

for k=1:ncond
    tmp=dat(:,k);
    jitt=(rand(1,length(tmp))+0.2).*0.2;
    scatter(k+jitt,tmp,dotsize,col,'filled','MarkerFaceAlpha',Alpha);
    hold on
end
...
for plt=1:ncond
    ...
    fill(tmpxb,tmpyb,col,'FaceAlpha',Alpha,'EdgeAlpha',1);
    hold on
