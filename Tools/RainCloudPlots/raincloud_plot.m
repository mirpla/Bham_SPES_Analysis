...
if box_on
    ...
    if ~isempty(SE)
        SEPOS = [Y(3)-SE box_pos(2) SE*2 box_pos(4)]
        h{7} = rectangle('Position', SEPOS);
        set(h{7}, 'EdgeColor', bxcl)
        set(h{7}, 'LineWidth', line_width);
        set(h{7}, 'FaceColor', [color,alpha]);
        set(h{7}, 'EdgeColor', cloud_edge_col);
        set(h{7}, 'LineWidth', line_width);
    end
    % 'box' of 'boxplot'
