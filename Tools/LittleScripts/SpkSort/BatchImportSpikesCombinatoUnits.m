...
for k = 1:length(subjID)
    ...
    for m = 1:length(dir(Direct))-2
        ...
        for c = 1:length(CSCfiles)
            ...
            SortSpikes.classes(:,3)     = double(h5read('sort_cat.h5','/matches'));
            SortSpikes.classes(:,4)     = double(h5read('sort_cat.h5','/distance'));
            SortSpikes.artifacts            = h5read('sort_cat.h5','/artifacts');
            SortSpikes.group            = h5read('sort_cat.h5','/groups');
            SortSpikes.types            = h5read('sort_cat.h5','/types');
            SortSpikes.classes(:,5)     = arrayfun(@(x) SortSpikes.group(2,SortSpikes.group(1,:) == SortSpikes.classes(x,2)),1:length(SortSpikes.classes(:,2)))';
          
            cd ..
            SorrtSpikes.times           = h5read(['data_CSC',num2str(c),'.h5'],'/pos/times');
            SorrtSpikes.spikes          = h5read(['data_CSC',num2str(c),'.h5'],'/pos/spikes');
            SorrtSpikes.artifacts          = h5read(['data_CSC',num2str(c),'.h5'],'/pos/artifacts');
            SorrtSpikes.thresh          = h5read(['data_CSC',num2str(c),'.h5'],'/thr');
            ...
            dum{c}.hdr                  = hdrCSC;
            dum{c}.waveform{1}(1,:,:)   = SortSpikes.spikes(:,SortSpikes.classes(:,1)+1); %spikes';
            dum{c}.timestamp{1}         = SortSpikes.times(SortSpikes.classes(:,1)+1)'; %cluster_class(:,2)'; % in s  (waveclus does it in ms usually)
