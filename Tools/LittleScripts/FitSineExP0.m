function [Phase0] = FitSineExP0(x,y,z) 
...
xp = linspace(min(x),max(x+500),length(x)+500);                     % Extend original data with extrapolation of 500 extra samples 
yp = fit(s,xp);

PhaInf  = angle(hilbert(yp));
Phase0 = rad2deg(PhaInf(1,z));
