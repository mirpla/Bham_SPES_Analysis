...
for ppp = 1:24
   ...
   % trial (sample 1358).
    dataset = Datapre{1,1}.trial{1,1} %randn(1,1639); 
    cfg             = [];
    data            = [];
    a = size(dataset,1);
    data.trial = {dataset(1,:)};
    data.time = {[0.001:1/fs:2.979]};
    data.label = {'sem'};
...
% %     
    ...
    freqSlid = 1024*diff(unwrap(tmpli))/(2*pi);
