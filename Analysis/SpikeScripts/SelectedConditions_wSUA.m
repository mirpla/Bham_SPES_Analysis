function [SUA, wSUA, TrialSelCond, WmTSCond] = SelectedConditions_wSUA(Data)
        ...
        for chan = 1:length(Data{1,k}{1,b})
            if ~isempty(Data{1,k}{1,b}{1,chan})
                for su = 1:length(Data{1,k}{1,b}{1,chan})
                    switch k
                        case 1
                            if b == 1
                                ...
                                WMAH = [];
                                for i = 1:length( wmStimHippAH )
                                    WMAH = [WMAH ; find(Data{1,k}{1,b}{1,chan}{1,su}.trialinfo ==  wmStimHippAH(i))];
                                end
                                WmTSCond{k,b,1} = WMAH;
                                
                                WMMH = [];
                                for i = 1:length( wmStimHippMH )
                                    WMMH = [WMMH; find(Data{1,k}{1,b}{1,chan}{1,su}.trialinfo ==  wmStimHippMH(i))];
                                end
                                WmTSCond{k,b,2} = WMMH;
                                
                                WMPH = [];
                                for i = 1:length( wmStimHippPH )
                                    WMPH = [WMPH; find(Data{1,k}{1,b}{1,chan}{1,su}.trialinfo ==   wmStimHippPH(i))];
                                end
                                WmTSCond{k,b,3} = WMPH;
                                
                                WM = [];
                                for i = 1:length( wmStimHipp )
                                    WM = [WM; find(Data{1,k}{1,b}{1,chan}{1,su}.trialinfo ==   wmStimHipp(i))];
                                end
                                WmTSCond{k,b,4} = WM;
                    ...
                    wSUA.Labels = {'WMAH','WMMH','WMPH','ALLWM'};
                    ...
                    for i = 1:size(WmTSCond,3)
                        cfg = [];
                        cfg.trials = WmTSCond{k,b,i};
                        wSUA.Conditions{1,k}{1,b}{1,chan}{1,su}{1,i} = ft_spike_select(cfg,Data{1,k}{1,b}{1,chan}{1,su});
                    end
                    wSUA.AllChan = SUA.AllChan;
                end
            end
        end
    end
